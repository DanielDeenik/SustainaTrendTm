...
def ai_first_strategy_hub():
    ...
    Features a completely redesigned UX following AI-first principles:
    ...
    - Finchat.io-inspired modern aesthetic
    """
    logger.info("AI-First Strategy Hub route called")
    
    try:
        # Get navigation context for template
        nav_context = get_context_for_template()
        
        # Render the AI-first strategy hub template
        return render_template(
            "strategy/ai_first_strategy_hub.html",
            page_title="AI-First Strategy Hub",
            active_nav="strategy",
            **nav_context
        )
    except Exception as e:
        logger.error(f"Error in AI-first strategy hub: {str(e)}")
        logger.error(traceback.format_exc())
        
        # Redirect to regular unified strategy hub in case of error
        flash("An error occurred loading the AI-First Strategy Hub. Falling back to standard view.", "warning")
        return redirect(url_for('strategy.unified_strategy_hub'))
